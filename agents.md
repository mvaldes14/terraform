...
├── Makefile                 # Automation commands for local development
...
This repository implements a comprehensive CI/CD pipeline to ensure code quality, security, and correctness.

### GitHub Actions Workflow

The `.github/workflows/terraform-ci.yml` workflow runs automatically on:
...
|-------|------|---------|----------------|
| **Format Check** | `terraform fmt` | Ensures consistent formatting | ❌ Blocks merge |
| **Validation** | `terraform validate` | Validates syntax and configuration | ❌ Blocks merge |
| **Linting** | TFLint | Detects errors and enforces best practices | ❌ Blocks merge |
| **Security Scan** | tfsec | Identifies security issues | ⚠️ Warning only |
| **Compliance Check** | Checkov | Policy and compliance validation | ⚠️ Warning only |
| **Documentation** | terraform-docs | Ensures module docs are up-to-date | ❌ Blocks merge |
| **Module Tests** | Terraform | Validates module examples | ❌ Blocks merge |

#### Workflow Jobs

1. **terraform-format**: Checks code formatting
   - Runs `terraform fmt -check -recursive`
   - Comments on PR if formatting issues found
   - Fails the check if format is incorrect

2. **terraform-validate**: Validates all configurations
   - Runs for each app and module directory
   - Performs `terraform init` and `terraform validate`
   - Matrix job for parallel execution

3. **tflint**: Static code analysis
   - Configured via `.tflint.hcl`
   - Checks naming conventions, deprecated syntax
   - Validates required versions and providers
...
5. **checkov**: Compliance and security
   - Policy-as-code validation
   - Infrastructure compliance checks
   - Uploads SARIF results

6. **terraform-docs**: Documentation validation
   - Ensures module README is current
   - Auto-generates documentation from code
   - Fails if docs are out of sync

7. **module-tests**: Module validation
   - Tests module examples
   - Ensures examples are valid and working

8. **summary**: Aggregates results
...
The `Makefile` provides convenient commands for local CI checks:
...
The `.tflint.hcl` file configures linting rules:
...
Atlantis provides automated Terraform workflows for pull requests:

**Configuration (`atlantis.yaml`):**
- Auto-plans on `.tf` file changes
- Configured for multiple workspaces (grafana, aws, github)
- Auto-merge enabled
- Deletes source branches after merge
...
6. Or merge PR for auto-apply (if configured)
...
1. **Pre-commit**: Gitleaks scans before commit
2. **CI Pipeline**: Automated scanning on PR
3. **GitHub Security**: SARIF uploads to Security tab

#### Security Scanning Tools

1. **tfsec**
   - Fast security scanner
   - AWS, Azure, GCP rule sets
...
1. Add job to `.github/workflows/terraform-ci.yml`:
```yaml
my-new-check:
  name: My New Check
...
3. Add to Makefile for local testing:
```makefile
...
**TFLint:**
Edit `.tflint.hcl` to enable/disable rules:
```hcl
...
**tfsec:**
Add inline comments to exclude specific checks:
```hcl
...
resource "aws_instance" "example" {
  #checkov:skip=CKV_AWS_8:Reason for skipping
  # ...
}
```
...
# - Add variable types
# - Fix naming conventions (use snake_case)
```
...
# 2. Add skip annotation with justification
# 3. Configure as soft-fail if acceptable risk
```
...
# Or manually for specific module
